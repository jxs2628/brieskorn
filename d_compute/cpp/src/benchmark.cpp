...
//   • compute_min_tau    (the main hot loop — O(N₀) iterations,
//                         N₀ = pqr − pq − qr − pr)
...
// ─────────────────────────────────────────────────────────────────────────────
// Benchmark one triple — fast (SIMD + cache) vs. scalar baseline
// ─────────────────────────────────────────────────────────────────────────────
static void benchmark_triple_fast(i64 p, i64 q, i64 r, const CeilTableCache& cache) {
    const i64 n0_raw = p*q*r - p*q - q*r - p*r;
    const i64 n0     = n0_raw > 0 ? n0_raw : 0;

    int reps = 1;
    if      (n0 <      1000) reps = 5000;
    else if (n0 <     10000) reps = 500;
    else if (n0 <    100000) reps = 50;
    else if (n0 <   1000000) reps = 5;

    SeifertData sd = brieskorn_seifert(p, q, r);

    i64 mt_scalar{};
    double ms_scalar = time_ms([&]{ return compute_min_tau(sd); }, reps, mt_scalar);
    i64 mt_fast{};
    double ms_fast = time_ms([&]{ return compute_min_tau_fast(sd, cache); }, reps, mt_fast);

    double per_scalar = ms_scalar / reps;
    double per_fast   = ms_fast   / reps;
    double speedup    = per_scalar / per_fast;

    std::cout << "  Σ(" << p << "," << q << "," << r << ")"
              << "  N_0 = " << n0
              << "   scalar " << std::fixed << std::setprecision(3) << per_scalar << " ms"
              << "   fast " << per_fast << " ms"
              << "   speedup " << std::setprecision(2) << speedup << "x"
              << "   (match: " << (mt_scalar == mt_fast ? "yes" : "NO") << ")\n";
}
...
int main() {
    ...
    // ── Fast (SIMD + cache) vs. scalar baseline ─────────────────────────────
    {
        i64 max_alpha = 0;
        for (const auto& t : triples) max_alpha = std::max(max_alpha, t.r);
        const CeilTableCache cache = build_ceil_tables((int)max_alpha + 1);

        std::cout << "\n── compute_min_tau_fast (SIMD + cache) vs. scalar ──\n";
        for (auto& t : triples)
            benchmark_triple_fast(t.p, t.q, t.r, cache);
    }

...
// reference vector of τ(0), τ(1), ..., τ(N₀), then check that tau_at
// returns the identical value for every m.  N₀ = pqr − pq − qr − pr is the
// scan upper bound used by compute_min_tau (Karakurt: τ is non-decreasing
// past N₀).  For Σ(2,3,5), N₀ = −1; we clamp to 0.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Test: compute_min_tau_fast == compute_min_tau (the SIMD kernel guard)
//
// The fast path uses int32 SIMD lanes inside process_chunk. This test pins it
// to the scalar baseline on a representative set of triples (small to large
// N₀) and on a broad fuzz over hundreds of pairwise-coprime triples.
//
// Failure here would mean the SIMD rewrite has gone wrong — either an
// overflow, a wrap-boundary bug, a tail-handling bug, or e0-sign edge case.
// ─────────────────────────────────────────────────────────────────────────────
static void test_min_tau_fast_matches_baseline() {
    std::cout << "\n── compute_min_tau_fast vs. baseline ─────────────────────\n";

    // Representative triples spanning small → large N₀.
    struct Triple { i64 p, q, r; };
    std::vector<Triple> triples = {
        {  2,   3,   5},   {  2,   3,  11},   {  2,   5,   7},
        {  5,   7,  11},   { 11,  13,  17},   { 23,  29,  31},
        { 47,  53,  59},   { 97, 101, 103},   {197, 199, 211},
    };

    int max_alpha = 0;
    for (const auto& t : triples) max_alpha = std::max<int>(max_alpha, (int)t.r);
    const CeilTableCache cache = build_ceil_tables(max_alpha + 1);

    for (const auto& t : triples) {
        SeifertData sd = brieskorn_seifert(t.p, t.q, t.r);
        i64 baseline = compute_min_tau(sd);
        i64 fast     = compute_min_tau_fast(sd, cache);
        check("fast vs. baseline Σ(" + std::to_string(t.p) + ","
...
// ─────────────────────────────────────────────────────────────────────────────
// Test: deterministic fuzz over many pairwise-coprime triples up to r=200.
//
// Compares the full d-invariant from the cached/fast path against the
// uncached scalar path on every coprime triple in the range. This exercises
// thousands of (e0, ω, α) combinations including non-trivial chunk
// boundaries and short scalar tails (chunks shorter than 8 steps).
// ─────────────────────────────────────────────────────────────────────────────
static void test_d_invariant_fuzz_to_200() {
    std::cout << "\n── d_invariant fuzz: coprime triples r ≤ 200 ─────────────\n";

    const int N = 201;  // exclusive upper bound
    const CeilTableCache cache = build_ceil_tables(N);

    int checked     = 0;
    int mismatches  = 0;
    i64 first_bad_p = 0, first_bad_q = 0, first_bad_r = 0;
    i64 first_bad_fast = 0, first_bad_slow = 0;
    ...
    for (int q = p+1; q < N; q++) {
        ...
        for (int r = q+1; r < N; r++) {
            ...
            // Sample sparsely beyond r > 60 to keep test runtime bounded;
            // dense for small triples where boundary cases matter most.
            if (r > 60 && (p + q + r) % 7 != 0) continue;

            DResult fast = d_invariant(p, q, r, cache);
            DResult slow = d_invariant(p, q, r);
            checked++;
            if (fast.d != slow.d) {
                if (mismatches == 0) {
                    first_bad_p = p; first_bad_q = q; first_bad_r = r;
                    first_bad_fast = fast.d; first_bad_slow = slow.d;
                }
                mismatches++;
            }
        }
    ...
    if (mismatches == 0) {
        tests_passed++;
        std::cout << "  PASS  fast == slow on " << checked
                  << " pairwise-coprime triples\n";
    } else {
        tests_failed++;
        std::cout << "  FAIL  " << mismatches << "/" << checked
                  << " mismatches; first at Σ(" << first_bad_p << ","
                  << first_bad_q << "," << first_bad_r << "): fast="
                  << first_bad_fast << " slow=" << first_bad_slow << "\n";
    }
}

// ─────────────────────────────────────────────────────────────────────────────
// Test: compute_min_tau_semigroup matches compute_min_tau on every
// pairwise-coprime triple with r < 80, plus a handful of larger / unbalanced
// triples that stress the lattice enumeration and the merge ordering.
//
// Catches off-by-ones in N₀, the dedup, and the +1/-1 event interleaving.
// ─────────────────────────────────────────────────────────────────────────────
static void test_min_tau_semigroup_matches_baseline() {
    std::cout << "\n── compute_min_tau_semigroup vs. baseline ────────────────\n";

    // Explicit (2,3,5) fallback (N₀ < 0).
    {
        SeifertData sd = brieskorn_seifert(2, 3, 5);
        check("semigroup Σ(2,3,5) fallback",
              compute_min_tau_semigroup(2, 3, 5), compute_min_tau(sd));
    }

    // Exhaustive sweep up to r < 80 (3,180 pairwise-coprime triples).
    const int N = 80;
    int checked    = 0;
    int mismatches = 0;
    i64 first_bad_p = 0, first_bad_q = 0, first_bad_r = 0;
    i64 first_bad_sg = 0, first_bad_base = 0;
    ...
    for (int q = p + 1; q < N; q++) {
        ...
        for (int r = q + 1; r < N; r++) {
            if (gcd64(p, r) != 1 || gcd64(q, r) != 1) continue;
            const i64 sg   = compute_min_tau_semigroup(p, q, r);
            const i64 base = compute_min_tau(brieskorn_seifert(p, q, r));
            checked++;
            if (sg != base) {
                if (mismatches == 0) {
                    first_bad_p = p; first_bad_q = q; first_bad_r = r;
                    first_bad_sg = sg; first_bad_base = base;
                }
                mismatches++;
            }
        }
    ...
    if (mismatches == 0) {
        tests_passed++;
        std::cout << "  PASS  semigroup == baseline on " << checked
                  << " coprime triples (r < " << N << ")\n";
    } else {
        tests_failed++;
        std::cout << "  FAIL  " << mismatches << "/" << checked
                  << " mismatches; first at Σ(" << first_bad_p << ","
                  << first_bad_q << "," << first_bad_r << "): semigroup="
                  << first_bad_sg << " baseline=" << first_bad_base << "\n";
    }

    // A handful of larger / unbalanced triples (slow on baseline but tractable).
    struct Triple { i64 p, q, r; };
    std::vector<Triple> spot = {
        {  2,   3, 199},   {  2,   5, 199},   {  2,   7, 197},
        {  3,   5,  97},   {  3,   7, 101},   {  5,   7, 103},
        { 11,  13,  97},   { 47,  53,  59},   { 97, 101, 103},
    };
    for (const auto& t : spot) {
        const i64 sg   = compute_min_tau_semigroup(t.p, t.q, t.r);
        const i64 base = compute_min_tau(brieskorn_seifert(t.p, t.q, t.r));
        check("semigroup Σ(" + std::to_string(t.p) + ","
              + std::to_string(t.q) + "," + std::to_string(t.r) + ")",
              sg, base);
    }
}
...
int main() {
    ...
    test_d_invariant();
    test_min_tau_fast_matches_baseline();
    test_min_tau_semigroup_matches_baseline();
    test_d_invariant_fuzz_to_200();
    test_consistency();
